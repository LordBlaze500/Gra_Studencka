...
function Recruiter($Troop, $Number)
{
   ...
   if ($Result == 1) header('Location: http://grastudencka.cba.pl/index.php?l=liquirstore&info=recruited');
   if ($Result == 2) header('Location: http://grastudencka.cba.pl/index.php?l=liquirstore&info=noresources');
   if ($Result == 3) header('Location: http://grastudencka.cba.pl/index.php?l=liquirstore&info=nospace');
}

if ($_GET['drunkard_number'] && $Liquirstore->Status_Getter() > 0)
{
   $Number = $_GET['drunkard_number'];
   Recruiter($Drunkard, $Number);
}

if ($_GET['clochard_number'] && $Liquirstore->Status_Getter() > 1)
{
   $Number = $_GET['clochard_number'];
